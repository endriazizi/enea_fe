...
_Generato: Sun, Nov  2, 2025  1:15:06 AM_
    ...
    <ion-item *ngFor="let o of filtered(); trackBy: trackById" (click)="openDetail(o)" (dblclick)="onCardTap(o)">
      <ion-label>
        <div style="display:flex;justify-content:space-between;gap:10px;">
          <div><b>#{{ o.id }}</b> — {{ o.customer_name || '—' }}</div>
        ...
        <!-- 🔧 Azioni veloci cambio stato (non cambia la struttura, solo una riga di bottoni) -->
  ...
  <div *ngIf="view()==='kanban'" style="display:grid; grid-template-columns: repeat(6, 1fr); gap:12px; padding:12px;" class="ion-hide-sm-down">
    <div *ngFor="let s of statuses">
      ...
      <div *ngFor="let o of kanbanColumns().get(s) || []; trackBy: trackById"
        ...
        <div (click)="openDetail(o)" style="cursor:pointer;">{{ o.customer_name || '—' }}</div>
        <div *ngIf="noteOf(o)" style="opacity:.85">{{ noteOf(o) }}</div>

        <!-- 🔧 Azioni veloci cambio stato -->
        <div style="display:flex; gap:6px; margin-top:8px; flex-wrap:wrap;">
    ...
    <ng-template>
      ...
      <ion-content>
        <div style="padding:12px" *ngIf="detail() as d">
          <p><b>Cliente:</b> {{ d.customer_name || '—' }}</p>
          ...
          <ion-list inset="true">
            <ion-item *ngFor="let it of d.items; trackBy: trackByItem">
              <ion-label>{{ it.name }} × {{ it.qty }}</ion-label>
              <ion-note slot="end">€ {{ it.price | number:'1.2-2' }}</ion-note>
            </ion-item>
          </ion-list>
        </div>
      </ion-content>
...
```
/* Fade-in morbido per cambi vista/lista */
.fade-in {
...
.filters .hours input[type="number"] {
  width: 84px;
  margin-left: 8px;
}
.filters .range {
...
.filters .range input[type="datetime-local"] {
  width: 190px;
}
...
/* Vista compatta */
.kanban-row.compact .kanban-card {
  --padding-start: 8px;
  --padding-end: 8px;
  border-radius: 12px;
...
<!-- ✅ Menu OVERLAY: CHIUSO di default, apribile con hamburger o swipe -->
